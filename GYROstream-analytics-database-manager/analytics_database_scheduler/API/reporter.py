...
class Reporter:
    ...
    # TODO: create a new function such that it takes a list of arguments which corespond to the ones supplied to this function
    def download_sales_report(
        ...
        return self._process_compressed_data(
            self.make_request("sales", command, credentials)
        )
    ...
    def download_financial_report(
        ...
        return self._process_compressed_data(
            self.make_request("finance", command, credentials)
        )
    ...
    def make_request(
        ...
        # TODO: Check for errors coming through (XML)
        # try:
        #     xml_data = ET.fromstring(response.text.strip())
        #     error_elm = xml_data.find("Error")
        #     if error_elm is not None:
        #         code = xml_data.find("Code")
        #         message = xml_data.find("Message")
        #         print(f"{code}:{message}")
        #         raise ReporterError
        # except ParseError:
        #     return response  # Content can be gzip

        # response.raise_for_status()
        return response

    # TODO: Convert this function such that it can account for the date columns and convert them appropiatley, instead of using the current method within the db_handler module
    @staticmethod
    def _process_compressed_data(response: requests.Response) -> Data:
        content_bytes = io.BytesIO(response.raw.read())
        content_type = response.headers.get("Content-Type")
        # print(response.headers)
        # print(content_bytes.read())
        df = pd.DataFrame()

        if (
            content_type == "text/plain;charset=iso-8859-1"
        ):  # NOTE: Need to adjust the make request to compensate for error messages, currently will use this (Unreliable)
            return df
        elif (
            content_type == "application/gzip"
        ):  # Check if its a zipfile and extract the necessary compressed file(s)
            ziped_file = zipfile.ZipFile(content_bytes)
            unziped_file = ziped_file.namelist()[0]
        ...
        # Normalize column names to be inserted into a database
        df.columns = [re.sub("[^a-zA-Z]+", "_", column).lower() for column in df]

        # Convert the date columns to datetime python objects
        for column in df.filter(regex=(".*(date).*")):
